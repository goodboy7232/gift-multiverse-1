...
import { ArrowLeft, QrCode, Wallet, CreditCard, Upload, CheckCircle, Copy, ImageIcon } from "lucide-react";
...
async function uploadOrderProof(token: string | null, orderId: string, data: any) {
...
export default function PaymentPage() {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => uploadOrderProof(token, orderId, data),
    onSuccess: () => {
    ...
    return (
      <div className="container mx-auto px-4 py-10 max-w-xl">
        ...
        <div className="text-center py-16 bg-card/50 border border-green-500/30 rounded-2xl">
          ...
          <p className="text-muted-foreground mb-1">Your payment proof is under review.</p>
          <p className="text-sm text-muted-foreground">Order #{orderId} · {order.giftCardName}</p>
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-xl">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground mt-1">Order #{orderId} · {order.giftCardName}</p>
        <p className="text-xl font-mono font-bold text-cyan-400 mt-2">${order.totalPrice.toFixed(2)}</p>
      </div>
      ...
      <p className="text-center text-xs text-muted-foreground/50 mt-8">
        Payment proofs are manually reviewed by our team. You will receive your gift card code once verified.
      </p>
    </div>
  );
}
